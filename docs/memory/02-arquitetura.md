...
src/
  ...
  index/               camada de índice GEAR v3 (bloco abaixo pré-Fase 1, pendente reconciliação Fase 8 — ccrs_calculator.py/ccrs_report.py já foram DELETADOS na Fase 1, ver item 25 de 05-decisoes-tecnicas.md; risk_calculator.py os substitui; hazard_scope.py e o risk_bands.py atual são Fase 3.1/3.2, descritos corretamente logo abaixo)
    risk_calculator.py  Risk_i,h = Hazard_i,h * Exposure_i * Vulnerability_i (Equação 1), por hazard, nunca somado entre hazards. Substitui ccrs_calculator.py (Fase 1). Ver item 25/27.
    hazard_scope.py     tabela H_b por bucket (Fase 3.1) — única fonte de verdade de quais combinações hazard/bucket existem: Hydro (ws,spei,precip,sv,iv) |5|, Thermal (ws,heat,precip) |3|, Wind (wind,) |1|, Solar (heat,precip,wind) |3|. Sem I/O. Ver item 35.
    risk_bands.py       RiskBand_i,h (Fase 3.2) — classificação discreta por hazard/bucket, lê hazard_scope.APPLICABLE_HAZARDS, nunca classifica combinação fora do H_b (HazardNotApplicableError). THRESHOLD_REGISTRY: Tier 1 absoluto (Water Stress, Extreme Wind/Wind binário) ou Tier 3 percentil (4 cortes, o mais baixo é diagnóstico, os outros 3 limitam as 4 bandas Low/Medium/High/Extreme). Substitui (deleta) o risk_bands.py antigo (WaterRiskBand/HeatRiskBand, CCRS). Ver item 36.
