...
| era5_wind_downloader + extreme_wind_processor | CDS `reanalysis-era5-single-levels`, `instantaneous_10m_wind_gust` horário, 1991-2020 (ERA5 histórico, **não** CMIP6 — sem eixo modelo/cenário) | `CDS_API_URL`/`CDS_API_KEY` | Por ano: `data/raw/climate/era5_wind/{país}/{ano}/annual_max.nc` (baixa→reduz a máximo anual→apaga bruto, um ano por vez — fix de disk-footprint 2026-09-12). Consolidado: `extreme_wind_gust_raw_{país}_{native,1km}.tif` (`compute_mean_annual_max_gust`/`ensure_raw_raster`, ainda não rodado para nenhum país em 2026-09-14) e `extreme_wind_gust_{país}_1km.tif` (normalizado, Min-Max por país — diagnóstico, não é o que alimenta `Risk_i,h`). Fluxo completo até `risk_calculator.py` (incluindo o passo de `normality_check` que decide `Tlog`/`Tlin`, ainda pendente) documentado em `docs/DECISIONS.md`, "GEAR v3 wind-into-risk_calculator pre-wiring audit" — não duplicado aqui |
