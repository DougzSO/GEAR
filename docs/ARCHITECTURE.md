...
## 11. Results-draft asset tree (author-approved, 2026-09-15)

The 16 figures/tables `docs/rework/GEAR_v3_RESULTS_DRAFT.md` names as
placeholders are generated by `src/reporting/results_draft/` into an
**additive** tree at `data/outputs/results_draft/` — approved by the author
over the alternative of reorganizing the existing `data/outputs/tables/` /
`maps/` / `diagrams/` layout, specifically to avoid touching the paths
`src/main.py` and every `src/visualization/` module already read/write.
`data/outputs/results_draft/` is gitignored like the rest of `data/outputs/`
(regenerable, not a source of truth); the generating scripts are the
committed artifact.

Layout: one numbered subfolder per RESULTS_DRAFT.md item,
`NN_slug/` (`01_correlation_gate/` … `16_threshold_tier_provenance/`), each
holding that item's CSV/PNG outputs, plus one `MANIFEST.md` at the tree root
(item → caption → real-data source → output files → status). Engineering
detail (module list, how to run, per-item data source, the pre-existing
`ccrs_calculator` import-name bug this package works around) is in
`docs/memory/04-scripts-comandos.md` and `docs/memory/06-areas-de-risco.md`,
not duplicated here — this section exists only because the folder-structure
decision itself was an author approval this document is the record of.