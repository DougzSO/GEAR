...
| Status | **Superseded (2026-09-16).** `Hazard_i,s` (the bucket-weighted per-technology composite this entry describes) no longer exists in the v3 pipeline: `risk_calculator.py` computes `Risk_i,h` per hazard, never combined or blended across hazards (Methods Section 1, Equation 1), and carries no `BUCKET_WEIGHTS`/`w_water`/`w_heat`/`w_drought` concept. This judgment call was real for the CCRS-era architecture and is kept here as a historical record, not as an open v3 item. |
| Superseded by | `docs/DECISIONS.md` [2026-09-11] "GEAR v3 Phase 1: Risk_i,h replaces the CCRS core (Equation 1)". |
| Reference | `docs/DECISIONS.md`, "SPEI drought term added to Hazard (spec item F closed)" (2026-09-04); `docs/LIMITATIONS.md`, "2026-09-16 — Legacy CCRS visualization/sensitivity layer removed" (sibling scope, the CCRS layer's code-level removal). |
...
| Status | **Implemented (2026-09-14).** Closed by the follow-up entry below once ERA5 acquisition finished and the empirical-skew protocol ran; `hazard_scope.PENDING_RISK_I_H_HAZARDS` is now an empty dict (verified directly in `src/index/hazard_scope.py`), and `risk_calculator.HAZARD_TERMS` includes `"wind"` (verified directly in `src/index/risk_calculator.py`). |
| Reference | `docs/DECISIONS.md`, "GEAR v3 Risk_i,h integration gap: precip wired in, wind still blocked on ERA5 acquisition" (2026-09-13); `docs/DECISIONS.md`, "GEAR v3 wind Risk_i,h integration: empirical transform result, PENDING_RISK_I_H_HAZARDS closed" (2026-09-14, the closing entry); `docs/LIMITATIONS.md`, "2026-09-12 — Extreme Wind: ERA5 gust input not yet downloaded for any country" (the acquisition-status entry this one narrows, not replaces); `src/index/hazard_scope.py`, `PENDING_RISK_I_H_HAZARDS`. |
...
| Status | **Superseded (2026-09-14, same day).** `compute_mean_annual_max_gust()` was run and `wind` was wired into `risk_calculator.HAZARD_TERMS`/`FROZEN_BOUNDS` later the same day, per the empirical-skew protocol (`docs/DECISIONS.md`, "GEAR v3 wind Risk_i,h integration: empirical transform result, PENDING_RISK_I_H_HAZARDS closed"). The 2026-09-13 Risk_i,h integration gap this entry's status line described as "open, unchanged" is now closed — see that entry's own status, updated in the same 2026-09-16 pass as this one. |
| Reference | `docs/DECISIONS.md`, "GEAR v3 Risk_i,h integration gap: precip wired in, wind still blocked on ERA5 acquisition" (2026-09-13, the entry whose blocker this closes); `docs/DECISIONS.md`, "GEAR v3 wind Risk_i,h integration: empirical transform result, PENDING_RISK_I_H_HAZARDS closed" (2026-09-14, the entry that closes the Risk_i,h gap referenced below); `docs/LIMITATIONS.md`, "2026-09-12 — Extreme Wind: ERA5 gust input not yet downloaded for any country" and "2026-09-13 — Extreme Wind: no Risk_i,h (Equation 1) yet, not only a RiskBand gap" (both narrowed by this entry; the Risk_i,h gap they describe is now implemented, see that entry's updated status); `src/index/hazard_scope.py`, `PENDING_RISK_I_H_HAZARDS["wind"]` (counts updated to match). |
...
## 2026-09-16 — Baseline for percentile-based hazards (Extreme Precipitation P95, SPEI-12 log-logistic fit): intra-scenario/pool-relative, not a fixed historical reference period

| Field | Content |
|---|---|
| Limitation | Extreme Precipitation's P95 wet-day threshold and SPEI-12's log-logistic calibration are both computed from each `(country, model, scenario)` cell's own 2041-2070 sample — there is no separate historical reference window (e.g. 1991-2020) pulled in to define "extreme" against. `extreme_precipitation_processor.py`'s `compute_extreme_precip_days` takes `np.nanpercentile` of the wet days already drawn from that one scenario's own loaded raster; `spei_processor.py`'s `_fit_loglogistic` fits per-calendar-month parameters from `D_accum`, the same single scenario's own 2041-2070 monthly series. Both then Min-Max normalize by pooling across GCM/scenario within that same future window, still not against a historical baseline. |
| Reason | Neither processor was designed to acquire or hold a second, historical-period dataset alongside its future-window one; the percentile-cutoff design (ETCCDI R95p convention, already Tier 3 per the 2026-09-11 HAZUS-MH entry above) was implemented as pool-relative from the outset. |
| Evidence tier | Direct code inspection (`src/processors/extreme_precipitation_processor.py`, `_compute_native`/`compute_extreme_precip_days`; `src/processors/spei_processor.py`, `_compute_native`/`_fit_loglogistic`) — not inferred. |
| Alternatives considered and rejected | None implemented — a historical-baseline variant was not attempted for either processor. |
| Status | Revisitable — would require acquiring and processing a historical (e.g. 1991-2020) `pr`/`tas` window per country/GCM specifically for calibration, not currently planned. Until then, cross-scenario or cross-GCM comparisons of "extreme" precipitation days or SPEI drought frequency are relative to each cell's own future-window pool, not to an absolute, scenario-independent extremity definition — the manuscript must state comparisons across SSPs as pool-relative, not as an absolute climate-extremity comparison. |
| Reference | `src/processors/extreme_precipitation_processor.py`; `src/processors/spei_processor.py`; `docs/LIMITATIONS.md`, "2026-09-11 — Extreme Precipitation: HAZUS-MH absolute threshold rejected, Tier 1 downgraded to Tier 3" (the sibling Tier-3/percentile-cutoff entry this one adds the baseline-window nuance to, not replaces). |

## 2026-09-16 — `upper_tail_padding_fraction` (Tier 3, nominal 0.05): un-documented-in-`LIMITATIONS.md` normalization parameter with measured Sobol influence

| Field | Content |
|---|---|
| Limitation | `Tlog(x) = -ln(1-x)` (the transform used for `LOG_TERMS` — `ws`, `heat`, `wind`, `spei`) pads the raw Min-Max upper bound by a fraction (`padded_hi = hi + pad * span`) before the log transform, so the pooled sample maximum never lands exactly on the `x=1` singularity. The fraction (`UPPER_TAIL_PADDING_FRACTION` in `src/index/normalization.py:223`, duplicated as `TLOG_UPPER_TAIL_PADDING_FRACTION` in `src/index/risk_calculator.py:541` to avoid a circular import) is nominal `0.05`, an author-chosen constant, not derived from data. Until this entry, it had no dedicated row in this file despite carrying a measured Sobol first-order sensitivity index S1 ≈ 0.195-0.237 on `Risk_i,h`/`PSAE` outputs — one of the more influential Tier-3 parameters in the model, per `docs/DECISIONS.md`'s Phase 6 Sobol entries and `docs/rework/GEAR_RESULTS_SUMMARY.md` Section 4.1. |
| Reason | A padding fraction is structurally required for any `-ln(1-x)` transform to stay finite at the pooled maximum; `0.05` was chosen so the padded value sits at `x ~= 0.952` (comment, `normalization.py:228-230`) — a value picked to keep the transformed range compact (`ln(1+1/0.05) ~= 3.045`), not calibrated against an external reference. |
| Evidence tier | Tier 3 (author-declared numerical constant); Sobol sensitivity indices (S1/ST) are Tier: empirical measurement of this project's own N0=1024 stratified run, not a literature value. |
| Alternatives considered and rejected | None recorded — no alternative padding value or transform-domain-handling strategy was evaluated against this one. |
| Status | Revisitable — flagged here explicitly as a sensitivity-analysis candidate, already exercised in the closed Phase 6 Sobol run (S1/ST reported), but the nominal value itself has not been reconsidered in light of those results. |
| Reference | `src/index/normalization.py:223-282` (`UPPER_TAIL_PADDING_FRACTION`, `transform_neg_log_minmax`); `src/index/risk_calculator.py:538-570` (`TLOG_UPPER_TAIL_PADDING_FRACTION`, duplicated for import-cycle reasons); `docs/DECISIONS.md`, Phase 6 Sobol sensitivity entries; `docs/rework/GEAR_RESULTS_SUMMARY.md` Section 4.1 (`upper_tail_padding_fraction` S1=0.1953, ST=0.1946 on `Risk_mean_overall`). |

## 2026-09-16 — Heterogeneous data sources and projection windows across hazards

| Field | Content |
|---|---|
| Limitation | The hazard set is not drawn from one uniform data pipeline: Water Stress / Storage Variability / Inflow Variability (`ws`/`sv`/`iv`) come from WRI Aqueduct 4.0's `future_annual` product (Google Earth Engine), a single pre-computed scenario column per year with **no per-GCM dimension in this codebase at all** and no bias-correction step (consumed as a downstream WRI product as-is), representing conditions "around 2050" (Aqueduct's "horizon 50") rather than an explicit 30-year averaging window. Heat / SPEI / Extreme Precipitation come from raw CDS CMIP6 output for exactly two GCMs (`gfdl_esm4`, `miroc6`, `src/config.py`'s `CMIP6_SOURCE_ID_CDS`), nearest-neighbour resampled with **no bias correction** (`src/downloaders/cds_tasmax_downloader.py`'s docstring: "there is no bias correction here, and a higher-order interpolator would fake spatial precision the data does not have"), over the explicit `2041-01-01`..`2070-12-31` window (`config.CMIP6_FUTURE_PERIOD`). Extreme Wind is a third, separate case (ERA5 historical reanalysis, 1991-2020 — already covered by its own 2026-09-11 entry above). |
| Reason | Each hazard's source was selected independently, by data availability, at the time its processor was built (Aqueduct for water-related indicators predates this project's CMIP6 acquisition layer; the 2-GCM CDS pipeline was a later, separate acquisition effort — see the GCM-pair selection entry above, 2026-09-11). No project-wide decision was made to standardize GCM count, bias-correction treatment, or projection-window length across hazard sources. |
| Evidence tier | Direct code inspection (`src/processors/water_stress_processor.py`, `src/downloaders/aqueduct_downloader.py`, `src/config.py`, `src/downloaders/cds_tasmax_downloader.py`) plus `docs/rework/GEAR_v3_methodology_nature_format.md` (Aqueduct window framing) — not inferred. |
| Alternatives considered and rejected | None — a uniform cross-hazard GCM/bias-correction/window standard was not evaluated; each processor was built and accepted independently. |
| Status | Final as a documented structural characteristic, not a pending fix — re-acquiring Aqueduct on a GCM-resolved basis, or bias-correcting the CDS layer, would each be a substantial new acquisition effort, not a parameter change. The manuscript must not aggregate or directly compare `ws`/`sv`/`iv` against `heat`/`spei`/`precip` as if they shared one climate-signal pipeline; any cross-hazard "climate risk" claim must be stratified by hazard source, or explicitly caveat the heterogeneity. |
| Reference | `src/processors/water_stress_processor.py`; `src/downloaders/aqueduct_downloader.py`; `src/config.py` (`CMIP6_SOURCE_ID_CDS`, `CMIP6_FUTURE_PERIOD`); `src/downloaders/cds_tasmax_downloader.py`; `docs/rework/GEAR_v3_methodology_nature_format.md` (Aqueduct horizon framing); `docs/LIMITATIONS.md`, "2026-09-11 — GCM pair (GFDL-ESM4, MIROC6): operational selection, not an ECS-based bounding design" (sibling entry, GCM-pair choice for the CDS-sourced hazards only). |

## 2026-09-16 — Static fleet: no plant retirement or addition, GEM-export-date roster aged forward to 2050

| Field | Content |
|---|---|
| Limitation | The plant inventory used for the 2050 projection is the GEM-snapshot roster as of its export date, filtered to `Status == "operating"` at that time (`src/downloaders/assets_validator.py`'s `split_by_status`, `OPERATING_STATUS`), carried forward unchanged into 2050 with only `age_factor`'s `age = YEAR_TARGET - commissioning_year` (`config.YEAR_TARGET = 2050`) changing. No code anywhere computes a retirement cutoff, drops a plant for exceeding an assumed operating lifetime, or adds new/planned capacity for the 2050 horizon — `"planned"`-status GEM rows are split out and never merged back into the main pipeline. |
| Reason | Simplification of scope: a fixed 2050 snapshot of the existing fleet, not an energy-planning scenario with capacity expansion/retirement modelling. |
| Evidence tier | Direct code inspection (`src/index/age_factor.py`, `src/index/risk_calculator.py`'s `load_plants`, `src/downloaders/assets_validator.py`) — not inferred. |
| Alternatives considered and rejected | None — a fleet-evolution (retirement/addition) model was never in scope for this project. |
| Status | Final as a documented scope boundary. Two compounding consequences worth naming together: (1) `age_factor` for hydro/coal plants commissioned well before 1980 is pushed toward its upper bound by a plant that, in reality, would very plausibly be retired or repowered by 2050 — inflating `age_factor` for the oldest cohort; (2) any plant commissioned after the GEM export date is entirely absent from the 2050 fleet, understating total capacity and skewing the roster toward older vintages, the same class of data-censoring artifact CLAUDE.md Section 9 already documents for the prior project's resilience-ceiling collapse. The manuscript must describe this as an "existing fleet aged to 2050" analysis, not an energy-planning or capacity-expansion scenario. |
| Reference | `src/index/age_factor.py` (`REFERENCE_YEAR`, `plant_age`); `src/config.py` (`YEAR_TARGET`); `src/downloaders/assets_validator.py` (`OPERATING_STATUS`, `split_by_status`); `CLAUDE.md` Section 9 (prior-project data-censoring precedent, same failure family). |

## 2026-09-16 — SPEI-12 PET: Thornthwaite (temperature-only), not Penman-Monteith or Hargreaves

| Field | Content |
|---|---|
| Limitation | `spei_processor.py` computes potential evapotranspiration with a fully in-house Thornthwaite implementation (`_thornthwaite_pet`) — monthly mean temperature (`tas`) plus an analytically computed day-length correction (Duffie & Beckman 2013 solar declination formula) — not Penman-Monteith (full energy balance: humidity, radiation, wind) and not Hargreaves (temperature range, `tasmin`/`tasmax`, cheaper than Penman-Monteith but still temperature-only in a different form). Thornthwaite tends to amplify projected drought signal under warming relative to energy-balance methods, since its PET term grows monotonically with temperature alone. |
| Reason | The module docstring states the choice explicitly: daily `tasmin` — needed for the cheaper Hargreaves alternative — is absent from the CDS `projections-cmip6` catalogue for `gfdl_esm4`/`ssp3_7_0`, which would silently drop that GCM/scenario cell if Hargreaves were used; Penman-Monteith's full humidity/radiation input set was not pursued for the same class of CMIP6-catalogue-availability reason across both configured GCMs. Thornthwaite is the one method computable uniformly for every configured GCM/scenario cell from data already acquired for other hazards (`tas`). |
| Evidence tier | Direct code inspection (`src/processors/spei_processor.py`, module docstring lines ~24-64, `_thornthwaite_pet`, `_day_length_hours`) — not inferred. |
| Alternatives considered and rejected | Hargreaves — rejected, would silently drop the `gfdl_esm4`/`ssp3_7_0` cell (missing daily `tasmin`); Penman-Monteith — not pursued, full input set not uniformly available across both configured GCMs on the CDS catalogue. |
| Status | Final for the current GCM pair — revisitable only if a future GCM/data-source swap makes `tasmin` (or full Penman-Monteith inputs) available uniformly across every configured model/scenario cell. The manuscript should cite the literature on SPEI's sensitivity to PET method choice (Thornthwaite vs. Penman-Monteith) when interpreting projected drought-frequency magnitudes, since the method itself has a known warming-amplification bias relative to energy-balance PET. |
| Reference | `src/processors/spei_processor.py` (module docstring, `_thornthwaite_pet`, `_day_length_hours`); `analysis/spei_catalog_check.md`; `docs/rework/GEAR_v3_methodology_nature_format.md` (Hargreaves rejection discussion). |
